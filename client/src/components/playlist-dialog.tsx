...
export function PlaylistDialog({ open, data, onClose, onConfirm }: Props) {
  ...
  const validateRange = useCallback(() => {
    const from = parseInt(rangeFrom, 10)
    const to   = parseInt(rangeTo || defaultRangeTo, 10)
    if (isNaN(from) || from < 1) { setRangeError('Start must be at least 1'); return false }
    if (isNaN(to) || to > total) { setRangeError(`End cannot exceed ${total}`); return false }
    if (from > to)               { setRangeError('Start cannot be greater than End'); return false }
    setRangeError(null)
    return true
        ...
        <DialogHeader>
          ...
          <DialogDescription asChild>
            <div className="mt-1 space-y-0.5 text-sm text-white/60">
              ...
              <p>{total} video{total !== 1 ? 's' : ''} found</p>
            </div>
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="rounded-lg border border-white/10 px-4 py-3 hover:bg-white/5">
              ...
              {mode === 'range' && (
                <div className="mt-3 flex items-center gap-3 pl-7">
                  ...
                  />
                  {resolvedEntries.length > 0 && !rangeError && (
                    <span className="text-white/40 text-xs">{resolvedEntries.length} video{resolvedEntries.length !== 1 ? 's' : ''}</span>
                  )}
                </div>
              )}
            ...
            <div className="rounded-lg border border-white/10 px-4 py-3 hover:bg-white/5">
              ...
              {mode === 'select' && entries.length > 0 && (
                <div className="mt-3 pl-2 space-y-2">
                  ...
                  {selected.size === 0 && (
                    <p className="text-xs text-yellow-400 pl-2">Select at least one video to continue</p>
                  )}
                </div>
              )}
            </div>
