...
export default function Dashboard() {
  ...
  const handlePlaylistConfirm = async (
    ...
    const result = await window.electronAPI.addPlaylistToQueue(entries, {
  ...
  const handlePlaylistDownload = async () => {
    ...
    try {
      ...
      const entriesToQueue = indicesToDownload.map(idx => ({
        video: videoInfo[idx],
        index: idx + 1,
        selectedFormat: selectedFormats[idx]
      }));

      const result = await window.electronAPI.addPlaylistToQueue(entriesToQueue, {
        savePath: settings?.download_path || settings?.downloadPath,
        createFolder: Boolean(settings?.create_playlist_folder ?? settings?.createPlaylistFolder ?? true),
        playlistTitle: playlistTitle || 'Playlist'
      });

      if (result.success) {
        setSuccessMsg(
          `${result.addedCount} item${result.addedCount === 1 ? '' : 's'} added to queue! Switch to Queue & History to track progress.`,
        );
        onGoToQueue?.();
      } else {
        onError({ message: 'Failed to add playlist to queue. Please try again.' });
      }
    } catch (err: any) {
      console.error('[Playlist Queue Error]', err);
      onError({ message: err.message || 'An error occurred while adding the playlist to the queue.' });
    } finally {
      setStartingPlaylist(false);
    }
  };
...
function SettingsPanel() {
  ...
  const loadBinaryUpdates = useCallback(async () => {
    ...
    try {
      ...
      setBinaryUpdates(updates.map((u: any) => ({ ...u, updating: false })));
    } catch (error: any) {
  ...
  useEffect(() => {
    ...
    loadAppVersion();
  }, [loadSettings, loadBinaryUpdates]);
          ...
          <div className="border-t border-border/50 pt-8 space-y-3">
            ...
            <div className="flex items-center space-x-3">
              <Switch
                ...
                onCheckedChange={(checked) => saveSetting('closeToTray', checked)}
              />
              <Label htmlFor="close-to-tray" className="text-sm cursor-pointer">
                Keep app running in system tray on close
              </Label>
            </div>
          </div>
