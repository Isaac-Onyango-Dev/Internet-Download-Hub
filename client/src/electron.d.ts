...
declare global {
  interface Window {
    electronAPI: {
      // Video Info
      fetchVideoInfo: (url: string) => Promise<{ success: boolean; data?: any; error?: string; meta?: any }>
      ...
      onYtDlpUpdateAvailable: (callback: (data: { currentVersion: string; latestVersion: string }) => void) => void
      onPlaylistDetected: (callback: (data: {
        title: string;
        count: number;
        entries: PlaylistEntry[];
      }) => void) => void
      addPlaylistToQueue: (entries: PlaylistEntry[], options: PlaylistOptions) => Promise<{ added: number; skipped: number }>
